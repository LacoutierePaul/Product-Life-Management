          ...
          {stocks.map((stock) => (
            <React.Fragment key={stock.idstock}>
              <tr className={stock.quantite < stock.seuil_minimal ? 'low-stock' : ''}>
                ...
                <td>{stock.unite}</td>
                <td>
                  {hasPermission('edit') && <button onClick={() => handleEditStock(stock)}>Modifier</button>}
                  {hasPermission('delete') && <button onClick={() => handleDeleteStock(stock.idstock)}>Supprimer</button>}
                  {hasPermission('order') && (
